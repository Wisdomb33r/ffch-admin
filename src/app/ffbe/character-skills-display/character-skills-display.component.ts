      ...
      this.subscription = forkJoin(observables).subscribe(results => {
        results.forEach((c, index) => {
            ...
            this.competencesContainers.push(
              new CompetencesComparingContainer(this.competences[index], FfbeUtils.isNullOrUndefined(c) ? undefined : Competence.produce(c), Competence.produce(this.competences[index]))
            );
          }
        );
      });
